...
module bnn_fcc_tb_v2 #(
    ...
    parameter string   BASE_DIR                                 = "/ecel/UFAD/miguel.sanchez1/BNN_Contest/Apple_comp/python",
    ...
    // Global cycle counter for steady-state throughput tracking
    ...
    initial begin : l_sequencer_and_driver
        ...
        for (int i = 0; i < num_tests; i++) begin
            ...
            for (int j = 0; j < current_img.size(); j += INPUTS_PER_CYCLE) begin
                ...
                // Hold valid high until the beat is accepted
                @(posedge clk);
                while (!data_in.tready) begin
                    @(posedge clk);
                end
        ...
        if (passed == num_tests)
            ...
            $error("FAILED: %0d out of %0d tests failed.", failed, num_tests);
